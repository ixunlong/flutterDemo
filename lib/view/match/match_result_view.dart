...
class _MatchResultViewState extends State<MatchResultView>
  ...
  final controller = Get.put(MatchResultController());
  ...
  Widget build(BuildContext context) {
    return GetBuilder<MatchResultController>(builder: (_) {
      return Stack(
        children: [
          Column(
            children: [
                ...
                children: [
                  Expanded(
                    child: DateTabBar(
                      ...
                      subTextStyle: TextStyle(fontSize: 12),
                      labelPadding:
              ...
              Expanded(
                child: ExtendedTabBarView(
                  controller: _tabController,
                  children: days
                      .map((e) => MatchResultListView(e, days.indexOf(e)))
                      .toList(),
                ),
              ),
            ],
...
class MatchResultListView extends StatefulWidget {
  final DateTime date;
  final int pageIndex;
  const MatchResultListView(this.date, this.pageIndex, {super.key});

  @override
  State<MatchResultListView> createState() => _MatchResultListViewState();
}

class _MatchResultListViewState extends State<MatchResultListView> {
  late MatchResultListController controller;

  @override
  void initState() {
    // TODO: implement initState
    super.initState();
    controller = Get.put(MatchResultListController(widget.date),
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<MatchResultListController>(
      // init: controller,
      tag: '${widget.pageIndex}',
      builder: (_) {
        return EasyRefresh(
