...
import 'package:sports/logic/data/basketball/bb_data_team_controller.dart';
...
class BbDataTeamPage extends StatefulWidget {
  BbDataTeamPage(this.leagueId, {super.key, required this.tag});

  int leagueId;
  final String tag;

  @override
  State<BbDataTeamPage> createState() => _BbDataTeamPageState();
}

class _BbDataTeamPageState extends State<BbDataTeamPage>
    with AutomaticKeepAliveClientMixin {
  late BbDataTeamController controller;

  @override
  Widget build(BuildContext context) {
    super.build(context);
    return GetBuilder<BbDataTeamController>(
      tag: widget.tag,
      initState: (state) {
        controller = Get.put(BbDataTeamController(widget.leagueId, widget.tag),
            tag: widget.tag);
      },
      didUpdateWidget: (oldWidget, state) {
        if (controller.visible) {
          controller = Get.put(
              BbDataTeamController(widget.leagueId, widget.tag),
              tag: widget.tag);
  ...
  Widget player(int index) {
    ...
    return GestureDetector(
      onTap: () {
        ...
        Get.toNamed(Routes.basketTeamDetail, arguments: data.qxbTeamId);
      },
      child: Container(
        height: 50,
