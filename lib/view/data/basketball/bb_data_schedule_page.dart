...
import '../../../logic/data/basketball/bb_data_schedule_controller.dart';
import '../../../res/colours.dart';
import '../../../res/styles.dart';
import '../../../widgets/loading_check_widget.dart';
import 'bb_data_schedule_item.dart';

class BbDataSchedulePage extends StatefulWidget {
  const BbDataSchedulePage({Key? key,this.leagueId, required this.tag}) : super(key: key);

  final int? leagueId;
  final String tag;

  @override
  State<BbDataSchedulePage> createState() => _BbDataSchedulePageState();
}
class _BbDataSchedulePageState extends State<BbDataSchedulePage>
    with AutomaticKeepAliveClientMixin{
  late BbDataScheduleController controller;

  @override
  Widget build(BuildContext context) {
    super.build(context);
    return GetBuilder<BbDataScheduleController>(
      tag: widget.tag,
      initState: (state){
        controller = Get.put(BbDataScheduleController(
            widget.leagueId,
                  ...
                  child: EasyRefresh(
                    ...
                    child: ListViewObserver(
                      ...
                      onObserve: (model){
                        ...
                        controller.setRound(controller.data?[model.firstChild?.index ?? 0].kindName);
                      },
                      child: MediaQuery.removePadding(
                        ...
                        child: ListView(
                          ...
                          children: List.generate(
                            controller.data?.length ?? 0, (index) => BbDataScheduleItem(data: controller.data?[index])),
                        ),
                      ),
                    ),
                  ),
          ...
          GestureDetector(
              onTap: () {
                ...
                controller.observerController.animateTo(
                    ...
                    alignment: 1/(controller.data![controller.currentItem].scheduleList!.length + 1));
              },
              child: Container(
